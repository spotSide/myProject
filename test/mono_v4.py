...
try:
    while True:
        ...
        # 심도 맵에서 특정 BGR 값 추출
        mask = cv2.inRange(result_frame, lower_bound, upper_bound)
        extracted_color = cv2.bitwise_and(result_frame, result_frame, mask=mask)

        # 원본과 심도 결과 병합
        stacked_frame = np.hstack((image, result_frame))  # 크기 맞춤 후 병합
        out_video.write(stacked_frame)

        # 실시간 화면 출력
        cv2.imshow('Monodepth with Extracted Color', stacked_frame)
