...
def unified_depth_pose_estimation(webcam_processor):
    ...
    compiled_model_depth = core.compile_model(model=model_depth, device_name="GPU")
    ...
    compiled_model_pose = core.compile_model(model=model_pose, device_name="GPU")
    ...
    try:
        while True:
            ...
            frame_with_all_resized = cv2.resize(frame_with_all, (1280, 1080))
            combined_frame = np.hstack((depth_frame_with_sections_resized, frame_with_all_resized))

            # 결합된 프레임 표시
            cv2.imshow("Depth and Pose Estimation (1080p)", combined_frame)
