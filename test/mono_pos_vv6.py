...
# Pose estimation 함수
def run_pose_estimation_with_webcam(webcam_processor, result_queue):
    try:
        while True:
            frame = webcam_processor.read_frame()  # 웹캠에서 프레임 읽기
            run_pose_estimation(frame, flip=False, use_popup=False)  # 포즈 추정 실행

            # 결과를 큐에 넣어 메인 스레드에서 출력
            result_queue.put(('pose', frame))
    except Exception as e:
...
def main():
    ...
    # Depth Estimation 스레드 실행
    depth_thread = threading.Thread(target=depth_main, args=(webcam_processor, result_queue))
    depth_thread.start()

    # Pose Estimation 스레드 실행
    pose_thread = threading.Thread(target=run_pose_estimation_with_webcam, args=(webcam_processor, result_queue))
    pose_thread.start()
